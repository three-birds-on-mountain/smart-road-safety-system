"""Load a local MOI A1 CSV file into PostgreSQL with English column names."""
...
DEFAULT_CSV_PATH = Path.home() / "moi_a1" / "A1_2025.csv"
...
class A1CSVDatasetLoader:
    ...
    def run(self) -> None:
        ...
        with psycopg2.connect(self.config.database_url) as conn:
            ...
            inserted = self._load_file(conn, csv_path, header, db_columns, insert_sql)
            conn.commit()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Load a stored MOI A1 CSV into PostgreSQL.")
    parser.add_argument(
        "--csv-path",
        type=Path,
        default=DEFAULT_CSV_PATH,
        help="Path to the A1 CSV file (default: ~/moi_a1/A1_2025.csv).",
    )
    parser.add_argument(
        "--table-name",
        type=str,
        default=DEFAULT_TABLE_NAME,
        help="Destination PostgreSQL table name (default: raw_moi_a1).",
    )
    parser.add_argument(
        "--database-url",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Value to store in the accident_level column (default: A1).",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
...
def main() -> None:
    ...
    loader = A1CSVDatasetLoader(config)
    loader.run()
