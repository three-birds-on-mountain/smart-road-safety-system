"""ETL：將 raw_moi_a1 台北市資料整併寫入 accidents。"""
...
VEHICLE_COLUMN = "accident_type_major_name"  # 依需求，視為車種來源欄位


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="將 raw_moi_a1 的台北市事故資料整併到 accidents。"
    )
    parser.add_argument(
        "--database-url",
    ...
    parser.add_argument(
        "--raw-table",
        default="raw_moi_a1",
        help="來源資料表名稱（default: raw_moi_a1）。",
    )
    parser.add_argument(
        "--accident-table",
        default="accidents",
...
def parse_occurrence_datetime(
    ...
    """將多個欄位組成 timezone-aware datetime (+8)。"""
...
def fetch_raw_rows(
    ...
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        ...
        rows = cur.fetchall()
    print(f"[INFO] 從 {source_table} 取得 {len(rows)} 筆台北市 A1 原始資料。")
    return rows
...
def purge_existing(conn, target_table: str) -> int:
    ...
    print(f"[INFO] 已清除 accidents 內 {deleted} 筆 A1 舊資料。")
    return deleted
...
def main() -> None:
    ...
    print(f"[INFO] 已寫入 {inserted} 筆台北市 A1 事故資料。")
