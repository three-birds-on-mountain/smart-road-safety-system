...
駕駛者在行駛過程中，當接近或進入交通事故熱點區域時，系統自動發出警示（語音播報 + 螢幕視覺提示），提醒駕駛者注意行車安全。

**Why this priority**: 這是系統的核心價值主張，直接關係到提升行車安全的目標。沒有這個功能，系統就失去存在意義。

**Independent Test**: 可以透過模擬GPS座標進入已知的事故熱點區域，驗證系統是否正確發出警示（包含語音和視覺提示）。

**Acceptance Scenarios**:

1. **Given** 用戶已開啟App並啟用GPS定位，**When** 用戶移動到距離事故熱點100公尺範圍內（根據用戶設定的提醒距離），**Then** 系統立即發出語音警示並在螢幕上顯示警示圖示與熱點資訊（包含事故等級、事故數量）
2. **Given** 用戶正在事故熱點區域內行駛，**When** 用戶離開該熱點區域（超出設定的提醒距離），**Then** 系統停止警示並清除視覺提示
3. **Given** 用戶進入事故熱點區域但已將該區域設為「已忽略」，**When** 用戶再次進入該區域，**Then** 系統不發出警示

---

### User Story 2 - 客製化警示設定 (Priority: P2)

用戶可以根據個人需求調整警示設定，包括：提醒距離（100m/500m/1km/3km）、關注的事故等級（A1/A2/A3）、語音開關等。

**Why this priority**: 不同駕駛者對警示的敏感度和需求不同，提供客製化設定可以提升用戶體驗，避免過度警示造成困擾。

**Independent Test**: 可以透過調整設定並模擬GPS移動，驗證系統是否依照設定的條件（距離、事故等級）正確觸發或不觸發警示。

**Acceptance Scenarios**:

1. **Given** 用戶在設定頁面，**When** 用戶選擇提醒距離為「1公里」並只關注「A1事故」，**Then** 系統儲存設定並在後續只在距離A1事故熱點1公里內發出警示
2. **Given** 用戶已關閉語音警示功能，**When** 用戶進入事故熱點區域，**Then** 系統僅顯示視覺警示，不播放語音
3. **Given** 用戶選擇關注所有等級事故（A1/A2/A3），**When** 用戶進入任何等級的事故熱點區域，**Then** 系統都會發出警示
...
- **事故熱點 (Accident Hotspot)**: 代表透過地理聚類分析產生的高風險區域，包含屬性：熱點中心座標（經度、緯度）、影響半徑（公尺）、包含的事故總數、各等級事故的數量與比例、最後更新時間
- **用戶警示設定 (User Alert Settings)**: 代表用戶的個人化警示偏好，包含屬性：提醒距離（100m/500m/1km/3km）、關注的事故等級（A1/A2/A3的組合）、語音警示開關、已忽略的熱點清單
