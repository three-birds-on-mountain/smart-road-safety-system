...
4. **Given** 用戶設定警示方式為「震動」，**When** 用戶進入事故熱點區域，**Then** 系統觸發震動並顯示視覺提示，但不播放任何聲音
...
- **FR-013**: 系統必須允許用戶選擇一種或多種警示方式組合（音效、震動、不提醒），當選擇「不提醒」時仍保留螢幕視覺提示但不發出聲音或震動
- **FR-021**: 系統必須支援多重警示方式同時觸發（例如：同時播放音效並觸發震動）
- **FR-019**: 系統必須允許用戶選擇事故時間範圍篩選（一年內、半年內、三個月內、一個月內）
...
- **用戶警示設定 (User Alert Settings)**: 代表用戶的個人化警示偏好，包含屬性：提醒距離（100m/500m/1km/3km）、關注的事故等級（A1/A2/A3的組合）、警示方式（音效/震動/不提醒）、事故時間範圍篩選（一年內/半年內/三個月內/一個月內）、已忽略的熱點清單
